...
3. [個別画面詳細](#個別画面詳細)
   ...
   - [データエクスポート / インポート](#データエクスポート---インポート)
...
| 画面 | パス (URL) | 主な機能 | 補足 |
|------|------------|----------|------|
| **トップページ** | `/` | 今日のワークアウトサマリ、BIG3 前回記録、体重・体脂肪概況 | アプリ起動時のハブ。API 呼び出しは `app/topPageUtil.ts` を経由 | 
| **ワークアウト入力** | `/workout` | エクササイズ選択、セット/レップ入力、ドロップセット自動ロジック | `src/app/workout/hooks/useWorkoutForm.ts` にて自動入力/バリデーション実装 |
| **体重記録** | `/weight` | 体重・体脂肪率入力、前日比・前月比計算 | 1日1件制限、既存データは上書き更新 |
| **グラフ** | `/chart` | 重量・体重・1RM 推移グラフ、期間別集計 | `src/app/chart/hooks/useChartData.ts` がデータ加工 |
| **設定** | `/settings` | Supabase 接続情報、テーマ切替、データ削除 | ユーザー設定はローカルストレージに永続化 |
| **データエクスポート** | `/export` | CSV/JSON エクスポート、バックアップ取得 | エクスポートはサーバーサイド API で生成 |
| **データインポート** | `/import` | CSV/JSON インポート、過去データ復元 | バリデーションは `src/app/api/import/` に実装 |
...
- **エクスポート** (`/export`)
  - CSV／JSON フォーマット選択
  - エクスポート対象: ワークアウト、体重、ドロップセットログ
  - ダウンロードはブラウザの `Blob` API を利用し、ファイル名は `training-log_YYYYMMDD.zip`。
- **インポート** (`/import`)
  - アップロードしたファイルの構造バリデーション（必須カラム、データ型）
  - 成功時は一括 INSERT、失敗時はエラーレポートをモーダルで表示
  - 既存レコードと重複した場合は `ON CONFLICT DO UPDATE` で上書き
...
## アクセシビリティ & UI ガイドライン
- **カラーコントラスト**: WCAG AA 以上（例: ダークモードの文字色 #E0E0E0、背景 #1E1E1E）
- **フォント**: Google Fonts `Inter`（500・600 ウェイト）を全体で使用
- **フォーカス順序**: キーボードだけで全操作可能、`tabindex` を明示的に設定
- **マイクロアニメーション**: ボタンホバー時に 0.2s のスケールアップ、入力エラー時にシェイクアニメーション
- **レスポンシブ**: モバイルは 1 カラムレイアウト、デスクトップは 2 カラムカード
...
| 日付 | 作成者 | 内容 |
|------|--------|------|
| 2026-05-25 | AI (Antigravity) | 初版 screen.md 作成。全画面一覧・遷移図・詳細仕様記述 |

---

以上が **training‑log** アプリケーションの画面仕様です。次のステップとしては、必要に応じて UI プロトタイプを作成し、デザインレビューを行うことを推奨します。
